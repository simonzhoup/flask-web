...
def resend_confirmation():
    ...
    send_email(current_user.email, u'验证邮箱', 'mail/confirm',
...
def forgot_password():
    ...
    if form.validate_on_submit():
        user = User.query.filter_by(email=form.email.data).first()
        token = user.generate_congirmation_token()
        send_email(user.email, u'【密码修改】验证邮件',
                   'mail/forgotpassword', user=user, token=token)
        flash(u'验证邮件已发送到你所注册的邮箱，请及时确认哦～')
        return redirect(url_for('main.index'))
...
def reset_email():
    ...
    if form.validate_on_submit():
        ...
        send_email(form.email.data, u'验证邮件', 'mail/reset_email',
                   user=current_user.username, token=token)
        flash(u'验证邮件已发送')
        return redirect(url_for('main.index'))
