...
class SetPassword(FlaskForm):
    ...
    password = PasswordField(u'输入新密码吧', validators=[
                             Required(), EqualTo('password2', message=u'两次密码好像不一致哦')])
...
class ForgotPassword(FlaskForm):
    email = StringField(u'告诉我你的注册邮箱', validators=[
...
class RePassword(FlaskForm):
    email = StringField(u'要更改密码的邮箱', validators=[
                        Required(), Length(1, 64), Email()])
    password = PasswordField(u'输入新密码吧', validators=[
                             Required(), EqualTo('password2', message=u'两次密码好像不一致哦')])
    password2 = PasswordField(u'确认新密码', validators=[Required()])
...
class ResetEmail(FlaskForm):
    email = StringField(u'告诉我你要使用的新邮箱', validators=[
                        Required(), Length(1, 64), Email()])
    submit = SubmitField(u'提交')
